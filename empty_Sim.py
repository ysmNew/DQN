...
obs_reward = -0.5
empty_reward = -0.2
goal_reward = 10.6 # 아이템 먹고 나올때 반드시 받는 back_reward를 커버하기 위해 0.6 추가
print('reward:' , move_reward, obs_reward, empty_reward, goal_reward)
    ...
    def get_reward(self, new_x, new_y, out_of_boundary):
        ...
        if any(out_of_boundary):
            ...
            # 그냥 움직이는 경우 
                ...
                if (self.grid[new_x][new_y] == 2) or (self.grid[new_x][new_y] == 3):
                    reward = empty_reward
                else:
                    reward = move_reward
...
# 현재의 grid를 텐서로 변경해주는 함수
    def get_state(self): 
        origin_grid = torch.from_numpy(self.grid.astype('int'))
        origin_grid = torch.unsqueeze(origin_grid,0) # tensor.size(1,10,9)
        origin_grid = torch.unsqueeze(origin_grid,0) # tensor.size(1,1,10,9)
        origin_grid = origin_grid.type(torch.float32)
