...
obs_reward = -0.5
back_reward = -0.2
goal_reward = 10.3 # 아이템 먹고 나올때 반드시 받는 back_reward를 커버하기 위해 0.3 추가
print('reward:' , move_reward, obs_reward, back_reward, goal_reward)
...
# 그리드(10,9)가 아니라 원핫인코딩(6,10,9)된 상태 반환
    ...
    def get_reward(self, new_x, new_y, out_of_boundary):
        ...
        if any(out_of_boundary):
            ...
            # 그냥 움직이는 경우 
                ...
                if (new_x,new_y) in self.actions:
                    reward = back_reward
                else:
                    reward = move_reward
        ...
        if self.done == True:
            if [new_x, new_y] == [9, 4]:
                if self.terminal_location == [9, 4]:
                    ...
                    render_cls.save_gif(self.ep)
                    render_cls.viewer.close()
                    display.stop()
        
        return s, action, reward, self.cumulative_reward, s_prime, self.done, goal_ob_reward
        
# 현재의 grid를 원핫인코딩해주는 함수
    def get_state(self): 
        origin_grid = self.grid.astype('int')
        oh_state = np.eye(6)[origin_grid]
        oh_state = torch.from_numpy(oh_state) # tensor.size(10,9,6)
        oh_state = torch.permute(oh_state, (2,0,1)) # tensor.size(6,10,9)
        origin_grid = torch.from_numpy(origin_grid) # tensor.size(10,9)
        origin_grid = torch.unsqueeze(origin_grid,0) # tensor.size(1,10,9)
        new_state = torch.concat([origin_grid, oh_state], dim=0) # tensor.size(7,10,9)
        new_state = torch.unsqueeze(new_state,0) # tensor.size(1,7,10,9)
