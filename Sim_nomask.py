...
##### array->tensor->array->tensor 수정필요
        ...
        new_state = torch.unsqueeze(origin_grid,0) # tensor.size(1,10,9)
        #new_state = torch.stack([origin_grid,origin_table]) # tensor.size(2,10,9)
        new_state = torch.concat([oh_state, new_state], dim=0) # tensor.size(7,10,9)
